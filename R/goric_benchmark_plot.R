plot.benchmark <- function(x, output_type = c("rgw", "rlw", "gw", "ld"), percentile = c(0.025, 0.975), 
                           x_lim = c(), ...) {
  ...
  if (any(percentile <= 0 | percentile >= 1)) {
  ...
  percentile_df <- aggregate(Value ~ Group, data = df_long, function(x) {
    c(lower = quantile(x, probs = percentile[1]), 
      upper = quantile(x, probs = percentile[2]), mean = mean(x))
  })
  ...
  p <- ggplot(df_long, aes(x = Value, fill = Group)) +
    ...
    geom_segment(aes(x = percentile_first_group_upper, 
                 ...
                 linetype = "solid", color = first_group_color, linewidth = 1) + 
    geom_segment(aes(x = 0, xend = 0, y = 0, yend = 0, color = paste0("[", paste(percentile*100, collapse = ", "), "]", "th percentiles")),
    ...
    scale_color_manual(name = c("", ""), 
                       values = c(setNames(first_group_color, paste0("[", paste(percentile*100, collapse = ", "), "]", "th percentiles")),
