summary.conRLM <- function (object, method = c("XtX", "XtWX"), correlation = FALSE, ...) {
  method <- match.arg(method)
  s <- object$scale
  coef <- object$b.constr
  ptotal <- length(coef)
  ...
  if (length(object$model.org$call$wt.method) && object$model.org$call$wt.method == "case") {
    rdf <- sum(wts) - p
    ...
    rdf <- n - p
    #w <- object$psi(wresid/s)
    w <- tukeyChi(wresid/s, cc = 4.685)
    S <- sum((wresid * w)^2)/rdf
    psiprime <- tukeyChi(wresid/s, cc = 4.685, deriv = 1)
    mn <- mean(psiprime)
  ...
  X <- 
    if (length(object$weights)) { 
      object$model.org$x * sqrt(object$weights)
    } else {
      object$model.org$x
    }
  if (method == "XtWX") {
