...
I am not in favor of cut-off points, but when you feel you need them to label the height/strength of the weights and/or their ratios, you can use benchmarks as proposed and discussed in 'Guidelines_GORIC-benchmarks' on https://github.com/rebeccakuiper/Tutorials.
...
For an overview of what type of hypotheses to include in the set and which failsafe hypothesis to use, see the decision trees in 'Decision trees - Choice hypotheses.pdf' on https://github.com/rebeccakuiper/Tutorials.
...
For an overview of what type of output to inspect and how to interpret, see the decision tree in 'Decision tree - Interpretation.pdf' on https://github.com/rebeccakuiper/Tutorials.
...
Then, I would advise you to evaluate it with its complement (the default option in the software), as opposed to the unconstrained hypothesis. Nevertheless, I will describe both situations next.
...
Notably, the ratio of GORIC(A) weights together with other output can also denote support for their boundary/border, which is discussed in the Special cases Section.

When inspecting a hypothesis versus its complement, one can interpret $1 - w_m = w_c$ as an error probability. That is, the probability that $H_m$ is not the best one is $w_c*100\%$. Note that in case of multiple hypotheses, especially in case of overlapping hypotheses, I would not advise on using this (conditional) error probability interpretation, since $w_m$ and thus $1-w_m$ depends on the other hypotheses in the set. 
...
results_1c
```
From this, you can conclude that $H_1: \mu_1 > \mu_2 > \mu_3$ is 
`r round(results_1c$result[1,7], 3)` $/$ `r round(results_1c$result[2,7], 3)` 
$\approx$ 
`r round(results_1c$result[1,7] / results_1c$result[2,7], 2)`
time more supported than its complement.  
Additionally, one could say that the probability that $H_1$ is not the best is `r round(100*results_1c$result[2,7], 0)`%.
...
Note: As will become clear in the Special cases Section, one may want to be able to find support for the boundary of these two hypotheses. Then, one should first check the ratio of fit/loglik weights (i.e. ratio of loglik.weights).
...
#### vs Hc together with boundary-hypothesis Heq
Your hypothesis of interest ($H_m$) does, by definition, not overlap with its complement ($H_c$). However, they do share their boundaries. One of the boundaries is setting all the inequalities to equalities ($H_{eq}$).
If you want to include that boundary-hypothesis, possibly to rule out support for that hypothesis, you can use the code below.
Note that the loglik weights can also denote support for a boundary (where at least one inequality is set to an equality). This is discussed in the Special cases section.
...
# Apply GORIC #
set.seed(123) 
results_1c_eq <- goric(fit, hypotheses = list(H1),
                    Heq = T)
results_1c_eq
```


### vs Unconstrained (but not advised)
...
# Apply GORIC #
set.seed(123) 
results_1u <- goric(fit, hypotheses = list(H1), comparison = "unconstrained")
results_1u
```
From this, you can conclude that $H_1: \mu_1 > \mu_2 > \mu_3$ is more supported than the unconstrained (since
`r round(results_1u$result[1,7], 3)` $/$ `r round(results_1u$result[2,7], 3)`
$>$ 1) and is, therefore, not a weak hypothesis.
...
Therefore, we can say that $H_1$ has (full) support: it has the maximum fit and is (by definition) more parsimonious than the unconstrained. Note that, now, we cannot say anything regarding the uncertainty (which we could in the case of using the complement as the failsafe, as done above).
...
Important: This option is not available in GORIC(A) software:  
Bear in mind that **the relative support of two informative hypotheses is independent from the choice of failsafe hypothesis**. So, if the goal is to compare informative hypotheses, one could just as well use the unconstrained (as discussed in the next subsection).
...
results_12u
round(results_12u$ratio.gw, 2)
```
From this, you can conclude that $H_1: \mu_1 > \mu_2 > \mu_3$ is not a weak hypothesis, since it is $>$ 1 times more supported than the unconstrained.
Therefore, $H_1$ can be compared to its competing hypothesis $H_2: \mu_1 < \mu_2 < \mu_3$. Note that $H_2$ is weak
`r round(results_12u$result[2,7], 3)` $/$ `r round(results_12u$result[3,7], 3)` $<$ 1 
or
`r round(results_12u$result[2,7], 3)` $<$ `r round(results_12u$result[3,7], 3)`; 
so, we already know that $H_1$ is better, but not yet how much better.

From the GORIC weights or, better, their ratios (cf. `results_12u$ratio.gw` - H1 vs. H2), you can conclude that $H_1$ is 
`r round(results_12u$ratio.gw[1,2], 0)`
times more supported than $H_2$. This denotes (almost) full support for $H_1$.  
...
Note: As will become clear in the Special cases Section, one may want to be able to find support for the boundary of the best hypothesis $H_1$ and the other hypotheses. Then, one should first check the ratio of fit/loglik weights (i.e. ratio of loglik.weights).
...
results_12
round(results_12$ratio.gw, 2)
```

From this, you can conclude that $H_1: \mu_1 > \mu_2, \ \mu_3$ is  
`r round(results_12$result[1,7], 3)` $/$ `r round(results_12$result[2,7], 3)` $\approx$ `r round(results_12$result[1,7] / results_12$result[2,7], 0)`
times more supported than $H_2: \mu_1 < \mu_2, \ \mu_3$.
...
In case of decision making, you may want to pre-specify a cut-off value $x$ for the ratio of GORIC(A) weights: if the ratio of GORIC(A) weights is larger than $x$, then I am willing to choose (to make policy based on) this best hypothesis. It can be hard to pre-specify $x$; unless there is already previous research perhaps. Alternatively, you could use the benchmark package for this, see 'Guidelines_GORIC-benchmarks.html' on https://github.com/rebeccakuiper/Tutorials. 
My advise is to create the hypotheses in such a way that finding a ratio of 1 (or higher) is enough evidence for the preferred hypothesis. This can still be hard to do, but in my opinion is easier than specifying $x$.  
I would also advise against overlapping hypotheses and using equality restrictions; for more detail, see the special cases discussed in the Special cases Section.

For example, let us assume that we compare an outcome (e.g., a standardized level of happiness) for three types of treatments: a new treatment (A), the established treatment (B), and a placebo treatment (P). You could evaluate $H_{1g}: \mu_A > \mu_B > \mu_P$ versus its complement $H_c$. Now, it can be hard to decide to go for Treatment A when $H_{1g}$ is 1 or perhaps 1.1 times more supported than $H_c$. Then, it could be handy (if possible) to specify the hypothesis in such a way that a ratio of (just over) 1 makes you willing to choose for Treatment A. You could specify a minimum difference between Treatments A and B and additionally state (as a check) that Treatment B (and then also A) does better than the placebo: $H_{1m}: \mu_A - \mu_B > .1, \ \mu_B > \mu_P$.
...
# Apply GORIC #
set.seed(123) 
results_1g <- goric(fit, hypotheses = list(H1g = H1g))
results_1g
round(results_1g$ratio.gw, 2)
```

From this, you can conclude that $H_{1g}: \mu_1 > \mu_2 > \mu_3$ is 
`r round(results_1g$result[1,7], 3)` $/$ `r round(results_1g$result[2,7], 3)` $\approx$ `r round(results_1g$result[1,7] / results_1g$result[2,7], 1)`
times more supported than its complement.
Since the ratio is larger than 1, $H_{1g}$ is the best.  
[Note: Since the fit/loglik weights differ (i.e., the ratio of loglik.weights is not close to 1), there is no support for the boundary of $H_{1g}$ and its complement - as discussed in the Special cases Section.]

You may now wonder whether this support is large enough to start using Treatment A instead of B. Therefore, you could -- if possible -- make you hypothesis more specific, such that a ratio of 1 or more is sufficient to select the hypothesis:
...
# Apply GORIC #
set.seed(123) 
results_1m <- goric(fit, hypotheses = list(H1m = H1m))
results_1m
round(results_1m$ratio.gw, 2)
```

From this, you can conclude that $H_{1m}: \mu_1 - \mu_2 > .1, \ \mu_2 > \mu_3$ is  
`r round(results_1m$result[1,7], 3)` $/$ `r round(results_1m$result[2,7], 3)` $\approx$ `r round(results_1m$result[1,7] / results_1m$result[2,7], 1)`
$>$ 1
times more supported than its complement.
...
[Note: Since the fit/loglik weights differ (i.e., the ratio of loglik.weights is not close to 1), there is no support for the boundary of $H_{1m}$ and its complement - as discussed in the Special cases Section.]
...
- When specifying a bound/threshold for a positive difference (e.g., the .1 in the example above), you should use the minimum difference you would like to find.  
Notably, if you use a higher bound (in case of an $>$ restriction) then needed, the complexity does not change, while the fit decreases; which leads to less support for your hypothesis of interest.

- In case you want to inspect an absolute difference, you can make use of the abs() function.  
E.g., $|\beta_1| > |\beta_2|$ can be reflected in the code - using b1 and b2 - by abs(b1) > abs(b2); or, when using bounds/thresholds, abs(b1) - abs(b2) > .1; and $|\beta_1 - \beta_2| >$ .1 can be reflected in the code by abs(b1 - b2) > .1.
<!-- NB  -->
<!-- abs(b1-b2)>.1 leidt naar ineq, zeg, b1-b2>.1 -->
<!-- Als juist about-eq, bijv, -.1<(b1-b2)<.1, dat leidt naar eq(!) in PT (en about-eq in LL) -->

Extra:  
If you have many parameters and you want to use their absolute value, you could use the following code:
```{r, results = FALSE}
# Example hypothesis
Hm <- "(HC_Anx, HC_Valu) > (HC_UncCon, HC_StudProg, HC_SelBel) > (HC_Plan, HC_Diseng, HC_TaskMa) > (HC_Pers, HC_FaiAvoi, HC_SelSab, HC_LearFo), 
(FBB_Anx, FBB_StudProg, FBB_Plan) > (FBB_Valu, FBB_FaiAvoi) > (FBB_LearFo, FBB_SelBel, FBB_SelSab) > (FBB_UncCon, FBB_Pers, FBB_TaskMa, FBB_Diseng), 
(TIL_Valu) > (TIL_FaiAvoi, TIL_SelBel, TIL_Plan, TIL_Anx, TIL_LearFo) > (TIL_SelSab, TIL_UncCon, TIL_TaskMa, TIL_Pers) > (TIL_StudProg, TIL_Diseng)"
#
# Function to write abs() around the parameter names
add_abs_to_names <- function(hyp) {
  names <- unlist(regmatches(hyp, gregexpr("[A-Za-z_]+", hyp)))
  for (name in unique(names)) {
    hyp <- gsub(paste0(name), paste0("abs(", name, ")"), hyp)
  }
  return(hyp)
}
#
# Apply function to hypothesis above
Hm_abs <- add_abs_to_names(Hm)
```
```{r}
Hm_abs
```
...
Note further that, in some cases, this implies support for the boundary/border of hypotheses, as will be discussed in a section later on.
...
```
From this, you can conclude that $H_1: \mu_1 > \mu_2 > \mu_3$ and $H_2: \mu_1 > \mu_2, \ \mu_3$ are not weak, since they are $>$ 1 times more supported than the unconstrained.
...
You could choose to investigate the support for this overlap, by evaluating $H_1$ versus its complement. We already did this in the first example subsection, there we found that $H_1$ is 
`r round(results_1c$result[1,7] / results_1c$result[2,7], 2)`
times more supported than its complement (with an error probability of 
`r round(100*results_1c$result[2,7], 0)`%). 
For future research, you could advise to evaluate $H_1$ versus its complement.
...
round(results_12u$ratio.gw, 2)
```
From this, you can conclude that $H_1: \mu_1 > \mu_2 > \mu_3$ and $H_2: \mu_1 > \mu_2, \ \mu_3$ are not weak, since since they are $>$ 1 times more supported than the unconstrained.
Therefore, $H_1$ and $H_2$ can be compared. 
...
From the GORIC weights `goric.weights` (or their ratios in `results_12u$ratio.gw` - H1 vs. H2), you can conclude that $H_2$ is 
`r round(results_12u$result[2,7], 3)` $/$ `r round(results_12u$result[1,7], 3)` 
$\approx$ 
`r round(results_12u$result[2,7] / results_12u$result[1,7], 2)`
more supported than $H_1$.  
Notably, if you would calculate the GORIC(A) weight for the set consisting of solely $H_1$ and $H_2$, you would obtain weights of 
`r round(results_12u$result[1,7], 3)` $/$ (`r round(results_12u$result[1,7], 3)` $+$ `r round(results_12u$result[2,7], 3)`) 
$\approx$ 
`r round(results_12u$result[1,7] / (results_12u$result[1,7] + results_12u$result[2,7]), 3)`
and 
1 $-$ `r round(results_12u$result[1,7] / (results_12u$result[1,7] + results_12u$result[2,7]), 3)` $=$
`r 1 - round(results_12u$result[1,7] / (results_12u$result[1,7] + results_12u$result[2,7]), 3)`,
respectively
(denoting the same relative support of course).

Since we know/see that $H_1$ and $H_2$ overlap, we can take it one step further. The support for $H_2$ indicates support for the non-overlapping part of $H_2$ and $H_1$. In this case, it is possible to specify the the non-overlapping part, namely $\mu_1 > \mu_2 < \mu_3$. If of interest, you could evaluate that versus its complement (as done below). For future research, you could advise to evaluate this non-overlapping part versus its complement (as done in the next subsection).
...
# Apply GORIC #
set.seed(123) 
results_n <- goric(fit, hypotheses = list(Hn = Hn))
results_n
```


### Support for boundary
When hypotheses do not overlap, there can mathematically still be overlap, namely the boundary/border of these hypotheses. This is always the case when evaluating an informative hypothesis versus its complement, but can also be the case for other pairs of hypotheses.  
...
Note: Depending on your interest and specification of your hypotheses, you may or may not be interested in checking the support for a boundary/border.
As a very simple example: When evaluating $\beta_1 > 0$ versus its complement (here, $\beta_1 < 0$), you may want to rule out that the support for $H_1$ is actually support for the boundary $\beta_1 = 0$. However, when evaluating $\beta_1 > 0.1$ versus its complement (here, $\beta_1 < 0.1$), you may not be interested in possible support for the boundary $\beta_1 = 0.1$; since you specified a minimum support to rule out support for the null $\beta_1 = 0$.
...
From the GORIC weights, we can conclude that $H_1$ is the best.
Before interpreting the ratio of GORIC(A) weights, one may want to check the fit/loglik values (or the ratio of their weights), to see whether there is support for a boundary. From the output, you can see that the fit values of $H_1$ and its complement resemble 
(`r round(results_1c$result$loglik[1], 3)`
vs
`r round(results_1c$result$loglik[2], 3)`)
and, thus, their loglik.weights are almost the same 
`r round(results_1c$result[1,5], 3)`
vs
`r round(results_1c$result[2,5], 3)`; 
i.e., their ratio is close to 1). Likewise, you can see that the GORICA weights resemble the penalty weights.
...
If of interest, one can also evaluate this boundary hypothesis versus its complement. Notably, such a boundary hypothesis contains at least one equality (here, there is one), which you may want to rephrase as an about-equality, as discussed in a next section; and exemplified in the the follow-up analysis in a next section. 
...
# Apply GORIC #
set.seed(123) 
results_b <- goric(fit, hypotheses = list(Hb = Hb))
results_b
```
...
One could also look into log-likelihood benchmarks, as discussed in 'Guidelines_GORIC-benchmarks' on https://github.com/rebeccakuiper/Tutorials.
...
He <- "D1 = D2 > 0, D3 > 0" # mu1 = mu2 > 0, mu3 > 0

# Apply GORIC #
set.seed(123) 
results_e <- goric(fit, hypotheses = list(He = He))
results_e
round(results_e$ratio.lw, 2) # ratio of loglik weights
round(results_e$ratio.gw, 2) # ratio of GORIC weights
```
...
When you look at the fit/loglik part, you see that both fit/loglik values resemble; that is, the ratio of loglik.weights is close to 1 (namely, 
`r round(results_e$ratio.lw[1,2], 2)`). 
Thus, you can conclude that there is support for the overlap/boundary of these hypotheses. 
...
```{r, message = TRUE, warning = FALSE}
# Equality versus its complement
Hae <- "-0.2 < D1 - D2 < .2, D1 > 0, D2 > 0, D3 > 0"
# -0.2 < mu1 - mu2 < .2, mu1 > 0, mu2 > 0, mu3 > 0

# Apply GORIC #
set.seed(123) 
results_ae <- goric(fit, hypotheses = list(Hae = Hae))
results_ae
round(results_ae$ratio.lw, 2) # ratio of loglik weights
round(results_ae$ratio.gw, 2) # ratio of GORIC weights
coef(fit)
```

From this, you can conclude that $H_1: -0.2 < \mu_1 - \mu_2 < .2,$ $\mu_1 > 0$, $\mu_2 > 0$, $\mu_3 > 0$ is 
`r round(results_ae$result[1,7], 2)` $/$ `r round(results_ae$result[2,7], 2)` 
$\approx$ 
`r round(results_ae$result[1,7] / results_ae$result[2,7], 2)` 
times more supported than its complement.
...
Note that the fit values of $H_1$ and its complement are not close: the ratio of loglik.weights is 
`r round(results_ae$result[1,5], 2)` $/$ `r round(results_ae$result[2,5], 2)` 
$\approx$ 
`r round(results_ae$result[1,5] / results_ae$result[2,5], 2)` 
(notably, this ratio changes with the choice of range). 
...
#### Note on about-equality vs absolute specification
An about-equality restriction should preferably be specified as a range restriction (e.g., $-0.2$ $<$ $\beta_1 - \beta_2$ $<$ $0.2$).
In that case, the fit is based on that range restriction; and the penalty then reduces to the penalty for an equality restriction, which make sense since this range restriction is meant as an equality.  
Notably, the same applies for using ranges not centered around 0 (e.g., $-0.1$ $<$ $\beta_1 - \beta_2$ $<$ $0.2$).

In an about-equality restriction, one specifies that a (function of) parameter(s) lies in between two values.
In theory, one could use the abs() function as well, but that is not advised. If you would use abs(b1 - b2) < .2, an inequality restriction is evaluated where the sign of the expression in abs() is made positive. That is, if b1 - b2 is positive, b1 - b2 < .2 is evaluated; if b1 - b2 is negative, then -(b1 - b2) < .2 is evaluated. In this case, the penalty is based on the inequality, not an equality restriction.  

I would advise on  
- expressing an about-equality via a range restriction, where you specify a range of values.  
- using the abs() function when you are interested in an inequality restriction for the absolute value of one or more parameters / for the strength of one or more predictors (e.g., abs(b1) > abs(b2); or, when using bounds/thresholds, abs(b1) - abs(b2) > .1).
...
In such cases, the sample size is not large enough to find convincing evidence. In such a case, the GORIC(A) weights will also denote this uncertainty, since none of the weights will be 1 (and perhaps the weights will even be close together).
...
When you believe the lower fit is solely because of a small sample or when you cannot create other theoretically meaningful hypotheses, you should suggest to inspect the same hypothesis with a larger sample size in future research.
...
# Apply GORIC #
set.seed(123) 
results_c <- goric(fit, hypotheses = list(H1 = H1))
results_c
round(results_c$ratio.gw, 2)
```

Here, we see that our informative hypothesis $H_1$ is the preferred one, but it does not have the best fit (ratio of loglik.weights is 
`r round(results_c$result[1,5], 2)` $/$ `r round(results_c$result[2,5], 2)` 
$\approx$ 
`r round(results_c$result[1,5] / results_c$result[2,5], 2)` 
$<$ 1).  
...
# Apply GORIC #
set.seed(123) 
results_i <- goric(fit, hypotheses = list(H1 = H1))
results_i
round(results_i$ratio.gw, 3)
```

Here, we see that our informative hypothesis $H_1$ is the preferred one, but it does not have the best fit (ratio of loglik.weights is 
`r round(results_i$result[1,5], 2)` $/$ `r round(results_i$result[2,5], 2)` 
$\approx$ 
`r round(results_i$result[1,5] / results_i$result[2,5], 2)` 
$<$ 1).  
...
# Apply bain
set.seed(123)
bain_c_f1 <- bain(fit, H1)
bain_c_f1
set.seed(123)
bain_c_f2 <- bain(fit, H1, fraction = 2)     
bain_c_f2
set.seed(123)
bain_c_f3 <- bain(fit, H1, fraction = 3)     
bain_c_f3
``` 
From this, you can see (from BF.c) that the equality hypothesis $H_1$ is approximately 
`r round(bain_c_f1$fit$BF.c[1], 0)`,
`r round(bain_c_f2$fit$BF.c[1], 0)`, and
`r round(bain_c_f3$fit$BF.c[1], 0)`,
times more supported than its complement when using a fraction of $1$, $2$, and $3$, respectively.
...
In terms of conclusions (i.e., there is support for $H_1$ or not), the output of GORIC(A) and `bain` are the same. In terms of the amount of support, `bain` renders more support for equality restrictions than the GORIC(A). In this example, this is good property. But: `bain` rendering more support for equality restrictions happens both when the equality restrictions are true/correct and when they are incorrect; as will be shown in the examples below. Bear in mind that I advise on using about-equality restrictions instead of equality restrictions (notably, the prior sensitivity (of `bain`) occurs for both types of restrictions).
...
# Apply bain
set.seed(123)
bain_i_f1 <- bain(fit, H1)
bain_i_f1

# Apply GORIC #
set.seed(123) 
results_i <- goric(fit, hypotheses = list(H1))
results_i
``` 
From the `bain` output, you can see (from BF.c) that the incorrect equality hypothesis $H_1$ is approximately 
`r round(bain_i_f1$fit$BF.c[1], 1)`
times more supported than its complement (when using a fraction of $1$).
From the GORIC output, you can see that the incorrect equality hypothesis $H_1$ is approximately 
$3.8$ 
`r round(results_i$ratio.gw[1,2], 1)`
times more supported than its complement.
...
In the case of $N = 1000$, GORIC already shows support for the correct complement; 
while `bain` still favors the incorrect equality restriction.
In the case of $N = 10000$, both methods show full support for the complement of $H_1$.
...
# Apply GORIC #
set.seed(123) 
results_i_N1000 <- goric(fit, hypotheses = list(H1))
results_i_N1000
``` 
...
# Apply GORIC #
set.seed(123) 
results_i_N10000 <- goric(fit, hypotheses = list(H1))
results_i_N10000
``` 
