...
vcov.MM <- function(X, resid0, resid, scale, ...) { 
  x <- X#as.matrix(object$model.org$x)
  ## set psi and chi constants
#  psi <- chi <- "bisquare"
  c.chi <- 1.54764
  c.psi <- 4.685061
  
  r0 <- resid0#as.numeric(object$resid0)
  r <- resid#as.numeric(resid(object))
...
#  p <- ncol(x)
  ...
  w  <- tukeyPsi(r.s, c.psi, deriv = 1) #robustbase:::Mpsi(r.s, cc = c.psi, psi = psi, deriv = 1)
  w0 <- tukeyChi(r0.s, cc = c.chi, deriv = 1) #robustbase:::Mchi(r0.s, cc = c.chi, psi = chi, deriv = 1)
  ...
  w0 <- tukeyChi(r0.s, cc = c.chi) #robustbase:::Mchi(r0.s, cc = c.chi, psi = chi)
