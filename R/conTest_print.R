print.conTest <- function(x, digits = max(3, getOption("digits") - 4), brief = FALSE, ...) {
  ...
  if (is.null(vnames)) { vnames <- paste("m", 1:length(object$b.constr), sep = "") }
    Amat <- object$Amat
    colnames(Amat) <- vnames
    out.rest <- cbind(Amat, rep("%*%colnames", nrow(Amat)),
                      c(rep(" == ", object$meq), rep(" >= ", nrow(object$Amat) -
                                                       object$meq)), object$bvec)
    rownames(out.rest) <- paste(1:nrow(out.rest), ":", sep = "")
    colnames(out.rest)[(ncol(Amat) + 1):ncol(out.rest)] <- rep(" ",
                                                               ncol(out.rest) - ncol(Amat))
...
#      out.test <- c(format(object$Ts, digits = digits), format(if (object$pvalue$pf.value < 1e-04 | object$pvalue$pchi.value < 1e-04) "<0.0001" else round(unlist(object$pvalue), 4), digits = 4))
      out.test <- c(format(object$Ts, digits = digits), format(round(unlist(object$pvalue), 4), digits = 4))
        names(out.test) <- c("Test statistic", "pf-value", "pchi-value")
      ...
      out.test <- c(format(object$Ts, digits = digits), format(if (object$pvalue < 1e-04) "<0.0001" else round(object$pvalue, 4), digits = 4))
        names(out.test) <- c("Test statistic", paste(boot, "p-value"))
    } else {
      out.test <- c(format(object$Ts, digits = digits), format(if (object$pvalue < 1e-04) "<0.0001" else round(object$pvalue, 4), digits = 4))
        names(out.test) <- c("Test statistic", "pt-value")
    }
    ...
    if (object$type == "B" && object$meq.alt == 0L) {
      cat("\n H0: all constraints true(>=)",
          "\n     vs. \n H1: at least one constraint violated (<)",
          "\n\n")
      print(out.test, quote = FALSE)
      if (!brief) {
      ...
      cat("\n H0: all constraints true(>= or =)",
          "\n     vs. \n H1: at least one constraint violated (<), some =-constraints maintained",
          "\n\n")
      print(out.test, quote = FALSE)
      if (!brief) {
        cat("\n\nConstraints on", vnames[colSums(!object$Amat == 0) > 0], "\n")
    ...
    if (object$type == "C") {
      cat("\n H0: at least one constraint not strictly true (<=)",
          "\n         vs. \n H1: all constraints strictly true (>)",
          "\n\n")
      print(out.test, quote = FALSE)
      if (!brief) {
        cat("\n\nConstraints on", vnames[colSums(!object$Amat == 0) > 0], "\n")
