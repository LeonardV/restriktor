...
#   # compute reduced R^2
#   R2.reduced <- 1 - sum(r^2) / sum((y - mean(y))^2)
#   # no weights and no intercept
#   if (is.null(w) & !attr(z$model.org$terms, "intercept")) {
#     R2.reduced <- 1 - sum(r^2) / sum(y^2)
#     # weights and intercept
#   } else if (attr(z$model.org$terms, "intercept") & !is.null(w)) {
#     R2.reduced <- 1 - sum(w * r^2) / sum(w * (y - weighted.mean(y, w = w))^2)
#   # weights and no intercept 
#   } else if (!(attr(z$model.org$terms, "intercept") | is.null(w))) {
#     R2.reduced <- 1 - sum(w * r^2) / sum(w * y^2)
#   }
#   
  if (is.null(w)) {
